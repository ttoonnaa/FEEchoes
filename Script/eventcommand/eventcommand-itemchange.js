...
// 法則：
// ・ユニットのアイテムが一杯であるときは、アイテムをストックに送るためのウインドウを表示。
// ・ただし、スキップまたは画像の非表示が有効である場合は、無条件にウインドウを表示。
// ・アイテムを増やす対象が敵ユニットである場合は、一杯になっても何も発生しない。
// ・アイテムを減らすコマンドは、重要アイテムも減らすことができる。
// ・ストックのアイテムが一杯であるときにアイテムを追加しようとするとアイテムを捨てることになるが、重要アイテムを捨てることはできない。
...
var ItemChangeEventCommand = defineObject(BaseEventCommand,
{
	...
	isEventCommandSkipAllowed: function() {
		// Startの押下によるスキップを許可するのは、単純にアイテムを入手できる場合のみである。
		// アイテムが一杯になるような場合はスキップは許可されない
...
var ItemChangeHelpWindow = defineObject(BaseWindow,
{
	...
	drawWindowContent: function(x, y) {
		// アイテムが一杯になったことの通知が複数ページにまたがると、違和感が生じるため、
	...
	_moveTop: function() {
		...
		if (input === ScrollbarInput.SELECT) {
			...
			if (this._isCancelAllowed(this._targetItem)) {
				// 入手アイテムをストックに送るか尋ねるウインドウのモードにする
				this._itemListWindow.enableSelectCursor(false);
	...
	_moveTradeQuestion: function() {
		if (this._questionWindow.moveWindow() !== MoveResult.CONTINUE) {
			if (this._questionWindow.getQuestionAnswer() === QuestionAnswer.YES) {
				// 所持アイテムをストックに格納し、空いた場所に入手アイテムを設定
				this._tradeItem();
				return MoveResult.END;
			}
			else {
...
var StockItemFull = defineObject(BaseItemFull,
{
	...
	_moveTop: function() {
		...
		if (input === ScrollbarInput.SELECT) {
			...
			// 重要でないアイテムの場合は、交換するかを確認するモードにする
			if (this._isSelectAllowed(item)) {
				this._itemListWindow.enableSelectCursor(false);
				this._questionWindow.setQuestionMessage(StringTable.ItemChange_TradeTitle);
				this._questionWindow.setQuestionActive(true);
			...
			// 選択したアイテムが重要でない場合は、捨てるかを確認するモードにする
			if (this._isCancelAllowed(this._targetItem)) {
				this._itemListWindow.enableSelectCursor(false);
