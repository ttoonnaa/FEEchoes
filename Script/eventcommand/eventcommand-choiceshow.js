...
var ChoiceShowEventCommand = defineObject(BaseEventCommand,
{
	...
	_prepareEventCommandMemberData: function() {
		...
		// _setScrollbarDataでisChoiceDisplayableが呼ばれるので、それより先行して呼び出す
		this._resetPreviousSelfSwitches(eventCommandData);
		
		this._setScrollbarData(eventCommandData);
		
		// 選択肢はスキップモードの場合でも表示したいため、強制的にスキップを停止する
		this.stopEventSkip();
	},
	
	_resetPreviousSelfSwitches: function(eventCommandData) {
		// デフォルトではこのメソッドを呼び出すことで、セルフスイッチは選択肢の表示の度にクリアしている。
		// 回想イベントでは選択肢そのものにセルフスイッチを指定できるため、
		// 前回の選択を敢えて考慮したい場合はクリアしない使い方がある。
