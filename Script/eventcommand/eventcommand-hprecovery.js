...
var HpRecoveryEventCommand = defineObject(BaseEventCommand,
{
	...
	_moveAnime: function() {
		...
		if (this._isAsyncAnime()) {
			// 現在のフレームインデックスがフレーム数の半分を超えたか調べる
	...
	_isAsyncAnime: function() {
		// HPの回復イベントは、経験値目的で頻繁に生じることがある。
		// たとえば、敵に「地形効果」の自然回復を利用させて何度も攻撃したり、
		// 意図的に敵の攻撃を受けて何度もアイテムで回復させたりするなど。
		// こうした行為を行っている場合は、HP回復もスムーズに完了するのが望ましいため、
		// このメソッドでtrueを返した場合は、回復アニメの再生が完了しない状態でも回復ウインドウを表示するようにしている。
		// これにより、アニメの再生が完了するまで待たされるということはなくなる。
