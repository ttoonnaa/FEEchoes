...
var UnitAllCommandEventCommand = defineObject(BaseEventCommand,
{
	...
	isEventCommandSkipAllowed: function() {
		// 「ダメージを与える」や「アイテムの増減」などはスキップを無効にできるため、falseを返す
		return false;
	},
	...
	_setUnitArray: function() {
		...
		// root.getCurrentSession().joinGuestUnitがまだ呼ばれていない状況、つまりroot.getMetaSession().getTotalPlayerListにゲストユニットが追加されてない場合は、
		// 意図的にゲストユニットをイベントコマンドのターゲットとして追加する。
		// たとえば戦闘準備画面のコミュニケーションイベントでゲストユニットを取り扱う場合に必要となる。
	...
	_isGuestListAllowed: function(filter) {
		...
		// 戦闘準備画面でないということは、既にゲストユニットはプレイヤーリストに追加されているので考慮しなくてよい
		if (root.getBaseScene() !== SceneType.BATTLESETUP) {
			return false;
		}
		
		// マップコマンドから「ユニット出撃」を選択して「Player Turn」のロゴが表示されるまでの間では、
		// 既にゲストユニットにはプレイヤーリストに追加されているので考慮しなくてよい
	...
	_setPosUnitArray: function() {
		...
		for (i = 0; i < count; i++) {
			...
			if (commandtype === EventCommandType.UNITSTATECHANGE) {
				// 非表示状態を有効にできるように、現在非表示なユニットも考慮する
...
var AllScrollFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_isCommandScrollable: function(targetUnit, command, commandType) {
		...
		if (commandType === EventCommandType.UNITREMOVE) {
			...
			result = this._isScriptExecuteScrollable(targetUnit, command);
		}
	...
	_isScriptExecuteScrollable: function(targetUnit, command) {
		return false;
	}
}
);
...
var AllEventFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_isCommandSkip: function(command) {
		return this._commandParent.isAllCommandSkip() || command.isGraphicsSkip();
	}
}
);
