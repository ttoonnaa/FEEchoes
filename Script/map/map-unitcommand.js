...
var UnitCommand = defineObject(BaseListCommandManager,
{
	...
	_moveOpen: function() {
		...
		if (object.moveCommand() !== MoveResult.CONTINUE) {
			// this._commandScrollbar.setActiveを呼び出さない。
			// PosSelectorによるキャンセルでも、確実にマウスカーソルを移動させる。
	...
	_moveEvent: function() {
		...
		if (this._capsuleEvent.moveCapsuleEvent() !== MoveResult.CONTINUE) {
			if (!UnitEventChecker.isCancelFlag()) {
				// キャンセルが発生していないということは、何らかの操作を行ったことを意味するため終了する
...
UnitCommand.Attack = defineObject(UnitListCommand,
{
	...
	_checkWeaponSelectionDisabled: function() {
		...
		// 「武器を1つしか持たない場合は攻撃時に武器選択しない」が有効かつ、装備可能な武器が1つしかない場合
	...
	_moveTop: function() {
		...
		if (input === ScrollbarInput.SELECT) {
			...
			if (this._weaponPrev !== this._weaponSelectMenu.getSelectWeapon()) {
				// 装備していた武器が変化したのでコマンドを再構築する。
				// たとえば、装備していた武器の追加スキルとして「盗む」が含まれるならば、コマンドから「盗む」が削除されなければならない。
				this.rebuildCommand();
			}
			return MoveResult.END;
		}
...
UnitCommand.Item = defineObject(UnitListCommand,
{
	...
	_moveTop: function() {
		...
		if (result === ItemSelectMenuResult.USE) {
			...
			// 装備武器が変わった、あるいはアイテムが捨てられたなどがあり得るから、コマンドを再構築する
			this.rebuildCommand();
			
			// アイテムを捨てた場合は行動を起こしたものとみなす
...
UnitCommand.Trade = defineObject(UnitListCommand,
{
	...
	_isTargetEnabled: function() {
		...
		// アイテム交換の相手がフュージョンされているユニットならば、フュージョンデータそのものがアイテム交換を許可するか調べる
	...
	moveCommand: function() {
		...
		if (SceneManager.isScreenClosed(this._stockItemTradeScreen)) {
			...
			if (resultCode === StockItemTradeResult.TRADEEND) {
				// ストック交換では交換を行っても直ちに待機にならないが、何らかの操作を行ったという印はつける
				this.setExitCommand(this);
				
				// ストック交換によって実行可能なコマンドが増えていることも考えられるため、再構築する
				this.rebuildCommand();
			}
...
UnitCommand.FusionAttack = defineObject(UnitCommand.Attack,
{
	...
	_completeCommandMemberData: function() {
		if (!this._isWeaponSelectDisabled) {
			// 戦闘開始前のPosMenuでのステータス表記時に、「フュージョン攻撃」の補正が加わるべきであるため、
			// この時点で呼び出している。
	...
	_startSelection: function(weapon) {
		if (this._isWeaponSelectDisabled) {
			// 「武器を1つしか持たない場合は攻撃時に武器選択しない」の場合は、このタイミングでstartFusionAttackを呼び出す
			FusionControl.startFusionAttack(this.getCommandTarget(), this._fusionData);
		}
	...
	_moveSelection: function() {
		...
		if (result !== MoveResult.CONTINUE) {
			if (this._isWeaponSelectDisabled) {
				// 「フュージョン攻撃」コマンドを選択したけれども、キャンセルして、通常の「攻撃」コマンドを選択することはよくある。
				// フュージョンデータが残らないように、クリアしておく。
				FusionControl.endFusionAttack(this.getCommandTarget());
			}
		}
