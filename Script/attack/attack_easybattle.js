...
var EasyMapUnit = defineObject(BaseObject,
{
	...
	setupEasyBattler: function(unit, isSrc, easyBattle) {
		...
		// 既に簡易戦闘が開始されており、この時点からスクロール値が変更されることは通常発生しないが
		// 念のため変更前の値を保存しておく。
	...
	_isScroll: function() {
		...
		if (this._xScrollPrev === root.getCurrentSession().getScrollPixelX() && this._yScrollPrev === root.getCurrentSession().getScrollPixelY()) {
			...
			// 現在スクロール値は、簡易戦闘へ突入した際とは異なっている可能性がある。
			// たとえば、戦闘時ユニットイベントの「場所注目」でスクロールが生じた場合などがある。
			// こうした場合、戦闘ユニットは現在スクロール位置に基づいて表示されるべきだからtrueを指定する。
			isScroll = true;
		}
