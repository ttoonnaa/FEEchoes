...
var RealBattle = defineObject(BaseBattle,
{
	...
	endBattle: function() {
		...
		// 戦闘終了後にbackBattleCycleでアニメの音が再生されるようなことを防ぐ
	...
	doHitAction: function() {
		...
		if (isHit) {
			...
			// 相手が待機状態である場合は、このメソッドによって待機が終了する
	...
	_createBattler: function() {
		...
		if (isRight) {
			// こちらの処理が実行される場合、unitSrcが自軍でunitDestが敵軍になっている
			...
			// こちらの処理が実行される場合、unitDestが自軍でunitSrcが敵軍になっている
	...
	_moveAnimation: function() {
		...
		if (this._isMotionBaseScroll) {
			// モーションや投げられた武器の現在位置を基準にスクロールさせる
			this._autoScroll.setScrollX(this.getActiveBattler().getFocusX());
		}
	...
	_checkBattleContinue: function() {
		...
		// 2つのバトラーの動作が停止し、同期エフェクトが終了しており、さらにUI(ゲージなど)がアニメーションしていないか調べる
	...
	_changeBattle: function() {
		...
		// モーションが動き始める場合は、そのモーションの位置を基準にスクロールするべきだからtrueを設定する。
		// この値はbattler.startBattlerでfalseに書き換えられることがある。
		// たとえば、モーションの第1フレームが魔法ループの開始の場合など。
...
var BaseBattler = defineObject(BaseObject,
{
	...
	moveAvoidOrDamage: function() {
		...
		if (this._checkLoopZone()) {
			...
			if (order.nextOrder()) {
				...
				// 「回避」または「ダメージ受け」のユニットが次に攻撃を行う場合は、一時的に何もしない状態にする。
	...
	isActionLast: function() {
		...
		// 「待機」のモーションタイプはループするため、isLastFrameで終了判定できない
	...
	setDamageState: function() {
		...
		if (damageId === -1) {
			// 「ダメージ受け」が設定されていない場合はモーションはそのままになる
...
var RealEffect = defineObject(BaseCustomEffect,
{
	...
	moveEffect: function() {
		...
		// EnemyOffsetControl.checkOffset呼び出しは、フレームが進んだ直後に行われるべきである。
		// そうでなければ、敵のずれが、1フレーム遅れて見えてしまう。
		if (this._isHitCheck) {
	...
	updateKeyPos: function() {
		...
		// 第1引数にrealEffectを指定した場合、エフェクトの拡大/縮小状態を考慮した位置を取得できる
	...
	_isCurrentFrameCheck: function() {
		// ダメージを与えるリアルバトルのエフェクトにおいて、第1フレームのサイズが他フレームのサイズと同一であればfalseを返してもよい
		return true;
	},
	
	// 「このフレームを命中判定にする」以後も縮小し続けるエフェクトを再生する場合は、このメソッドはtrueを返したほうがよい。
	_isUpdatable: function() {
		...
		if (motionCategoryType === MotionCategoryType.AVOID || motionCategoryType === MotionCategoryType.DAMAGE) {
			// 回避やダメージ受けの時は、モーションが後退するような動作を、アニメーションエディタで組み込んでいる場合がある。
			// このような場合、モーションと同時にエフェクトも後退するのは違和感が生じるため、エフェクトの位置は更新するべきではない。
			return false;
		}
...
var RealAutoScroll = defineObject(BaseObject,
{
	...
	setScrollX: function(x) {
		// 自動スクロールが有効な場合は、moveAutoScrollのみスクロール値を変更できる
		if (this._isAutoStart) {
			return;
		}
...
var UIBattleLayout = defineObject(BaseObject,
{
	...
	_drawColorAnime: function(rangeType) {
		...
		// 現在、表示中の全てのエフェクトの背景アニメを確認する。
		// ダメージポップアップもBaseCustomEffectを継承しているので、エフェクトとみなされる。
		for (i = 0; i < count; i++) {
			effect = effectArray[i];
			motion = effect.getAnimeMotion();
			// ダメージポップアップはgetAnimeMotionでnullを返す
	...
	_isDamageEffectScaling: function() {
		// ダメージエフェクトの拡大率が一定でない場合はtrueを返す
		return false;
	},
	
	_isDamageEffectAsync: function() {
		// 「武器エフェクト」を設定している場合は、ここでtrueを返すと戦闘のテンポがよくなる
		return false;
	},
	...
	_isFullFixedFocus: function(realBattle) {
		...
		// 「戦闘時に視点を固定する」が有効であり、なおかつ、境界線が接近している場合、アスペクト比率を維持した拡大を許可する。
		// この機能は画面解像度が16:9(1280×720など)の場合に、見栄えがよくなる。
		// 境界線がデフォルトの192から292に変更されていれば、ヒドラのような巨大モーションもバトルレイアウトに収まる。
		// 通常、敵の姿は自軍がスクロールして接近しなければ見えないが、境界線をずらしておくことで、スクロールなしで敵の姿を見えるようになる。
	...
	_drawBackgroundFixedFocus: function() {
		...
		if (this._scrollBackground.isScrollable()) {
			...
			pic = this._getBackgroundImage();
			if (pic !== null) {
				// root.getGameAreaWidth(Height)を指定することで、背景画像をゲーム画面全体に表示できる
...
var ClipingBattleContainer = defineObject(BaseBattleContainer,
{
	...
	_createMapCache: function() {
		...
		// 戦闘時に毎回マップやユニットを描画することのないよう、キャッシュに描画しておく
...
// このオブジェクトは16:9解像度でも、バトルレイアウトの崩れることなく表示できるために用意されている。
// このオブジェクトを使う場合、キャッシュに書き込まれたバトルレイアウトを本来の比率を維持したまま拡大するため、画面の両端には隙間ができる。
// その隙間を埋めるために、バトルレイアウト内で背景を描画するのではなく、バトルレイアウトが描画される前に背景を描画している。
// 言い換えると、背景画像とバトルレイアウトを分離して描画している。
// この手法は、画面解像度と一致する戦闘背景を所有している場合に、背景の拡大が生じずに綺麗に見える利点がある。
// たとえば、画面解像度が1280×720であれば、戦闘背景も1280×720に揃えれば、背景は拡大されない。
// このオブジェクトは、背景全体を画面に表示するため、スクロールという概念を持たない。
// よって、「戦闘時に視点を固定する」が有効でない場合は、このオブジェクトは使用されない。
var FixedFocusBattleContainer = defineObject(BaseBattleContainer,
{
	...
	pushBattleContainer: function() {
		...
		// 既に背景を描画済みだから、バトルレイアウト内で背景が描画されないようにする
	...
	_getSurfaceWidth: function() {
		// RealBattleArea.WIDTHは、変更されていないと仮定している
