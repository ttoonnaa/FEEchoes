...
var BaseMessageView = defineObject(BaseObject,
{
	...
	drawMessageView: function(isActive, pos) {
		...
		if (!isActive) {
			// 顔画像がウインドウをはみ出る場合を考え、drawFaceより先行して描画する
...
// BaseMessageViewを継承したオブジェクトを管理する(このオブジェクト自体は、BaseMessageViewを継承しない)
var FaceView = defineObject(BaseObject,
{
	...
	drawMessageView: function() {
		...
		if (this._isMessageWindowFixed()) {
			...
			if (view !== null) {
				// getMessageBottomPosを呼び出すことで、ウインドウが下側に表示される
