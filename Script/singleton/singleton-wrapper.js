...
var RetryControl = {
	...
	register: function() {
		...
		// 一時的なセーブデータを内部で保存する。
		// スクリプトエラー時に一時的なセーブデータが存在している場合は、それがロードされてゲームが再開する。
		// game.iniで「自動再開」が有効でない場合は、この関数は何もしない。
...
var DefineControl = {
	...
	getTextPartsHeight: function() {
		// ItemRenderer.getItemHeightが返す値と同一
		return 30;
	},
...
var MediaControl = {
	...
	clearMusicCache: function() {
		// 事前にresetMusicListを呼び出して無音にする
		this.resetMusicList();
		
		// メモリ内に存在するBGMデータを開放する
