...
var TextRenderer = {
	...
	drawAlphaText: function(x, y, text, length, color, alpha, font) {
		root.getGraphicsManager().drawText(x, y, text, length, color, alpha, validateFont(font));
	},
...
var UnitRenderer = {
	...
	// このメソッドとGraphicsRenderer.getGraphicsとの違いは、カラーを明示的に指定する点
...
// ただし、背景のように描画元の概念がない画像は、GraphicsRendererを使用しないこともある。
...
var ContentRenderer = {
	drawLevelInfo: function(x, y, unit) {
		...
		// ユニットが敵である場合は、ゲームオプションで「敵撃破時の経験値をクラスの追加経験値にする」が設定されているか調べる
...
// ウインドウ画像などを毎回構築するのはコストがかかるため、
// 一度作成したらキャッシュとして保存しておき、以後使い回すようにしている。
var CacheControl = {
	_cacheArray: null,
	...
	createCacheGraphics: function(width, height, pic) {
		...
		cache.height = height;
		cache.pic = pic;
		// root.getGraphicsManager().createCacheGraphicsとroot.getMaterialManager().createImageで作成した画像は、
		// その画像が一切参照されなくなった時点で開放される。
		// インポートされたリソースは、一切参照されずかつ一定期間経過した時点で開放される。
