...
var CurrentMap = {
	...
	prepareDivisionAreaArray: function() {
		...
		// このため、マップの表示範囲を一定数に分割し、ReinforcementCheckerでその範囲内に援軍が入っているかを順に確認する。
		y = 0;
		yEnd = 0;
		for (; y < height;) {
			yEnd += this.getRow();
...
var MapLayer = {
	...
	resetMapLayer: function() {
		// this._mapなどをしている場合は、ここでnull初期化する
	},
...
var MapHpControl = {
	// ParamBonus.getMhpを呼び出すことで、内部でunit.saveMhpが呼ばれる。
	// ユニットの最大HPをキャッシュするために、ParamBonus.getMhpを呼び出す必要がある。
	updateHp: function(unit) {
		var mhp = ParamBonus.getMhp(unit);
		
		// 現在HPが最大HPを超過していないかを確認する。
		// この現象は、HPを下げるステートを付与した場合に発生する。
		if (unit.getHp() > mhp) {
			unit.setHp(mhp);
		}
	},
...
// HPを描画するのではなく、どのようにHPを描画するかをgame.exe側に伝える。
