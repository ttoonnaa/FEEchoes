...
var ItemUseScreen = defineObject(BaseScreen,
{
	...
	_changeData: function(index) {
		...
		// 切り替え前ユニットと切り替え後ユニットで、ストックに表示されるアイテムが異なるケースが考えられる。
		// たとえば、ユニットが使用できないアイテムは、ストックに表示されない仕様になっているなど。
		// このような場合、itemIndexがストックアイテム数を超過していないか確認が必要となる。
		if (itemIndex >= this._stockItemWindow.getItemScrollbar().getAvailableArray().length - 1) {
			itemIndex = 0;
			yScroll = 0;
		}
		
		// ストックウインドウは以前の位置を維持する
