...
var BonusInputWindow = defineObject(BaseWindow,
{
	...
	setUnit: function(unit) {
		...
		if (this._isExperienceValueAvailable()) {
			// レート10で、500ボーナス所持の場合は、最高で50exp入手できる
	...
	// ユニット毎にレートを変動したい場合は、this._unitを参照
	_getRate: function() {
		// レートが1の場合は、1ボーナスの消費で1expを入手できる。
		// レートが10の場合は、10ボーナスの消費で1expを入手できる。
		return root.getUserExtension().getExperienceRate();
	},
	
	// 何らかの理由で特定ユニットに経験値を与えたくない場合は、this._unitを参照
	_isExperienceValueAvailable: function() {
		// 所持ボーナスがレートより低い場合は、経験値を与えることはできない
