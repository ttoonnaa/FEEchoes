...
var EndingScene = defineObject(BaseScene,
{
	...
	setSceneData: function() {
		// エンディングに入った時点で、直ちにゲームのクリアを通知する。
		// これ以降の処理で強制終了が発生した場合に、クリアが無効になるようなことをなくす。
...
var EndingSaveFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_completeMemberData: function(endingScene) {
		...
		// 「シーンの変更」で「エンディング」が実行される前に、
		// 「セーブ画面呼び出し」で「クリア済みとして保存」を選択していた場合は、
		// isCompletedSaveFlagがtrueを返すので、クリアデータをセーブファイルに保存する。
