...
var DemoFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_enableSoundVolume: function(isOn) {
		...
		if (DataConfig.isDemoMapSoundEnabled()) {
			// デモマップで効果音が有効化されている場合は、処理を続行しない
			return;
		}
...
var ClearPointFlowEntry = defineObject(BaseFlowEntry,
{
	...
	_isScreenDisplayable: function() {
		...
		// ClearPointFlowEntryの前にDifficultyFlowEntryが実行され、
		// そこではroot.getSceneController().initializeMetaSessionが呼ばれている。
		// つまり、難易度データはgame.exe内で初期化されており、root.getMetaSessionで取得できる。
		if (!(root.getMetaSession().getDifficulty().getDifficultyOption() & DifficultyFlag.NEWGAMEPLUS)) {
			return false;
		}
		
		// 値が0より大きい場合、一度でもゲームをクリアしたことを意味する
